...
#     """rotate right by 8 bits on 32-bit word"""
#     n &= 31
#     return ((x >> n) | ((x & 0xFFFFFFFF) << (32 - n))) & 0xFFFFFFFF

# def make_Te_tables():
#     Te0 = [0]*256; Te1=[0]*256; Te2=[0]*256; Te3=[0]*256; Te4=[0]*256
#     for x in range(256):
#         s = S_BOX[x]
#         t = ((mul(s,2)  << 24) |
#              (s         << 16) |
...
# def make_Td_tables():
#     Td0 = [0]*256; Td1=[0]*256; Td2=[0]*256; Td3=[0]*256; Td4=[0]*256
#     for x in range(256):
#         s = INV_S_BOX[x]
#         t = ((mul(s,14) << 24) |
#              (mul(s,9)  << 16) |
...
#         Td2[x] = rotr32(t, 16)
#         Td3[x] = rotr32(t, 24)
#         Td4[x] = (s<<24)|(s<<16)|(s<<8)|s
#     return Td0, Td1, Td2, Td3, Td4


def _rijndael_key_setup_enc(key: bytes) -> tuple[list[int], int]:
    """Compute the AES encryption key schedule.

    Args:
        key: Raw AES key bytes of length 16, 24, or 32.

    Returns:
        The expanded decryption round key and the AES round count.
    ...
    # Determine Nr and schedule size
    Nr = {128: 10, 192: 12, 256: 14}[key_bits]
    rk = [0] * (4 * (Nr + 1))

    # Initial words
    n_words = key_bits // 32
    rk[:n_words] = struct.unpack(">" + "I" * n_words, key)
    ...
    if key_bits == 128:
        ...
        for i in range(10):
            ...
            rk[offs + 5] = rk[offs + 1] ^ rk[offs + 4]
            rk[offs + 6] = rk[offs + 2] ^ rk[offs + 5]
            rk[offs + 7] = rk[offs + 3] ^ rk[offs + 6]
            offs += 4
        return rk, Nr
    ...
    if key_bits == 192:
        ...
        for i in range(8):
            ...
            rk[offs + 7] = rk[offs + 1] ^ rk[offs + 6]
            rk[offs + 8] = rk[offs + 2] ^ rk[offs + 7]
            rk[offs + 9] = rk[offs + 3] ^ rk[offs + 8]
            if i == 7:
                break
            rk[offs + 10] = rk[offs + 4] ^ rk[offs + 9]
            rk[offs + 11] = rk[offs + 5] ^ rk[offs + 10]
            offs += 6
        return rk, Nr
    ...
    for i in range(7):
        ...
        rk[offs + 9] = rk[offs + 1] ^ rk[offs + 8]
        rk[offs + 10] = rk[offs + 2] ^ rk[offs + 9]
        rk[offs + 11] = rk[offs + 3] ^ rk[offs + 10]
        if i == 6:
            break
        temp = rk[offs + 11]
        rk[offs + 12] = (
        ...
        rk[offs + 13] = rk[offs + 5] ^ rk[offs + 12]
        rk[offs + 14] = rk[offs + 6] ^ rk[offs + 13]
        rk[offs + 15] = rk[offs + 7] ^ rk[offs + 14]
        offs += 8
    return rk, Nr


def _rijndael_key_setup_dec(key: bytes) -> tuple[list[int], int]:
    """Compute the AES decryption key schedule.

    Args:
        key: Raw AES key bytes.

    Returns:
        The expanded encryption round key and the AES round count.
    """
    ek, Nr = _rijndael_key_setup_enc(key)
    dk = ek[:]
    i = 0
    j = 4 * Nr
    while i < j:
        dk[i + 0], dk[j + 0] = dk[j + 0], dk[i + 0]
        dk[i + 1], dk[j + 1] = dk[j + 1], dk[i + 1]
        dk[i + 2], dk[j + 2] = dk[j + 2], dk[i + 2]
        dk[i + 3], dk[j + 3] = dk[j + 3], dk[i + 3]
        i += 4
        j -= 4
    # apply inverse mixcolumns to all middle round keys
    p = 4
    for _ in range(1, Nr):
        for k in range(4):
            w = dk[p + k]
            dk[p + k] = (
                Td0[Te4[(w >> 24)] & 0xFF]
                ^ Td1[Te4[(w >> 16) & 0xFF] & 0xFF]
                ^ Td2[Te4[(w >> 8) & 0xFF] & 0xFF]
                ^ Td3[Te4[(w) & 0xFF] & 0xFF]
            )
        p += 4
    return dk, Nr
...
class _AESContext:
    """Internal AES primitive: key expansion + single-block operations.

    This object performs AES key scheduling and provides ``encrypt_block`` and
    ``decrypt_block`` for 16-byte blocks. It serves as a backend for CBC/ECB
    mode wrappers.
    """

    __slots__ = ("_rk_enc", "_rk_dec", "nr", "_out_enc", "_out_dec")

    def __init__(self, key: bytes) -> None:
        """Initialize AES key schedules for encryption and decryption.

        Args:
            key: AES key of length 16, 24, or 32 bytes.
        ...
        self._rk_enc, self.nr = _rijndael_key_setup_enc(key)
        self._rk_dec, _ = _rijndael_key_setup_dec(key)

        self._out_enc = bytearray(16)
        self._out_dec = bytearray(16)

    def encrypt_block(self, plaintext: bytes) -> bytes:
        """Encrypt a single 16-byte block.

        Args:
            plaintext: A 16-byte block.
        ...
        s0, s1, s2, s3 = struct.unpack(">IIII", plaintext)
        s0 ^= rk[0]
        s1 ^= rk[1]
        s2 ^= rk[2]
        s3 ^= rk[3]
        ...
        struct.pack_into(">IIII", self._out_enc, 0, c0, c1, c2, c3)
        return bytes(self._out_enc)

    def decrypt_block(self, ciphertext: bytes) -> bytes:
        """Decrypt a single 16-byte block.

        Args:
            ciphertext: A 16-byte encrypted block.
        ...
        s0, s1, s2, s3 = struct.unpack(">IIII", ciphertext)
        s0 ^= rk[0]
        s1 ^= rk[1]
        s2 ^= rk[2]
        s3 ^= rk[3]
        ...
        struct.pack_into(">IIII", self._out_dec, 0, p0, p1, p2, p3)
        return bytes(self._out_dec)
...
def new(
    ...
    """Create a new AES cipher in the requested mode.

    Args:
        key: AES key of length 16, 24, or 32 bytes.
        mode: One of ``MODE_ECB`` or ``MODE_CBC``.
        iv: Initialization vector for CBC mode. Must be 16 bytes. If ``None``,
            a zero IV is used (for education/testing).

    Returns:
        A mode object implementing ``encrypt()`` and ``decrypt()``.

    Raises:
        ValueError: If the key size is invalid, the mode is unknown, or the
            IV length is incorrect.
    """
    ctx = _AESContext(bytes(key))
    ...
    if mode == MODE_CBC:
        ...
        return CBCMode(
            encrypt_block, decrypt_block, block_size, None if iv is None else bytes(iv)
        )
